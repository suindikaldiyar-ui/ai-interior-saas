...
export const DEMO_OPENINGS: Opening[] = [
  ...
  {
    /*
     * КОРОБ ВЕНТИЛЯЦИИ ПОД ПОТОЛКОМ.
     *
     * В панельных домах по потолку идёт балка или короб, и шкафы под ним
     * в полную высоту не встают. Стоит он НАД ВЕРХНИМ РЯДОМ, а не над
     * колоннами: над колонной любой выступ съедает кладовку над
     * холодильником целиком, и демонстрация показывала бы не «шкаф ниже»,
     * а «модуля нет» — а клиент спрашивает именно про высоту.
     */
    id: 'beam-1',
    kind: 'beam',
    fromCornerMm: 2400,
