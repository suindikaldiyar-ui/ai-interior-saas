...
export type PaletteColor = {
  ...
  typical: boolean;
  /*
   * ЦЕНЫ ЗДЕСЬ НЕТ (слой 52). Поле `price` хранилось и ни на что не
   * влияло: фасад в цвете палитры смета считает ставкой цеха «Фасады»,
   * и карточка цвета никакой цены не показывала. Доехать до сметы оно
   * могло бы только новой строкой — и тогда у отправленных предложений
   * с этим цветом сдвинулась бы сумма, которую клиенту уже назвали.
   * Цена позиции каталога остаётся в `catalog_items`: её читает декор
   * корпуса (`carcassCatalog`).
   */
};
