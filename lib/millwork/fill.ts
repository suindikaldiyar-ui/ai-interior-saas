...
/**
 * ВЫСОТА МОДУЛЯ ПОД ВЫСТУПОМ НА ПОТОЛКЕ.
 *
 * Одно место на продукт: и запрет «не выше низа ригеля», и требование
 * «вплотную к нему». Вторая формула рядом развела бы их на первой же
 * правке — а это ровно те миллиметры, по которым цех пилит боковину.
 *
 * ВЕРХНИЙ РЯД ПРИЖИМАЕТСЯ К РИГЕЛЮ, А НЕ ПРОСТО ПОМЕЩАЕТСЯ ПОД НИМ.
 *
 * Шкаф висит, и место над ним — это щель, которую не вымыть и в которую
 * ничего не положить. Мебельщик её не оставляет: под выступом шкаф
 * делают по месту. Здесь стоял только `Math.min` — предел, а не цель, —
 * и в стандартном режиме шкаф оставался ровно 720 мм:
 *
 ...
 * он считал от потолка и обрезался этим же `min`. Совпадение результата,
 * а не общая формула.
 *
 * НАПОЛЬНОЕ И ПЕНАЛЫ ТЯНУТЬ НЕЛЬЗЯ: они стоят на полу, и высота у них
 * своя. Для них выступ остаётся пределом.
 *
 * Антресоль сюда не приходит: её ветка выше и её высоту выбирает
 * заказчик. Дотянуть её до ригеля значило бы переписать его выбор.
 *
 ...
 * разрывает ряд), а не эта функция; напольный, раздавленный ниже
 * полезного минимума, отказывает словами (`beamCrushes`).
 */
function capByCeiling(
  ...
  const room = Math.max(0, ceiling - bottomFromFloorMm(unit, run));

  const hangsOnWall =
    (unit.kind === 'upper' || unit.kind === 'corner_upper') && unit.section !== 'mezzanine';

  return hangsOnWall ? room : Math.min(heightMm, room);
}
