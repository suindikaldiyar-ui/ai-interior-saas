...
export type ModuleVariantSpec = {
  ...
  drawerCount?: number;
  /**
   * Сколько створок, когда их число — свойство ВАРИАНТА, а не ширины.
   *
   * «Две дверцы» — это две дверцы и на 600 мм тоже; у подъёмника створка
   * одна при любой ширине: ради широкого цельного фасада он и нужен, а
   * механизм считается на фасад. Не объявлено — решает ширина.
   */
  doorCount?: number;
...
export const MODULE_VARIANTS: Record<ModuleVariantKind, ModuleVariantSpec> = {
  ...
  /*
   * ДВЕ СТВОРКИ НАВЕРХУ — ОТДЕЛЬНЫЙ ВАРИАНТ, КАК И ВНИЗУ.
   *
   * Внизу `door_two` был всегда, наверху его не было вовсе: на антресоли
   * шириной 900 мм полотно шло одно, и «число створок» выбрать было
   * нечем. Петель вдвое больше, и в цеху это другая позиция — поэтому
   * вариант, а не «дверца пошире».
   *
   * Он становится доступен и обычному верхнему шкафу: это тот же ряд и
   * та же мебель, и прятать его там значило бы завести правило «наверху
   * двустворчатых не бывает», которого в отрасли нет.
   */
  upper_door_two: {
    kind: 'upper_door_two',
    title: 'Две створки',
...
export function applyVariant(unit: Module, kind: ModuleVariantKind): Module {
   ...
   */
  /*
   * ЧИСЛО СТВОРОК ОБЪЯВЛЯЕТ САМ ВАРИАНТ, А НЕ ЕГО ИМЯ.
   *
   * Здесь стояли два сравнения с ключами (`door_two` — две, `upper_lift` —
   * одна), и третий такой вариант их не наследовал: `upper_door_two`
   * проваливался в «по ширине» и на модуле ровно 600 мм давал ОДНУ
   * створку — то есть «две створки» ничего не меняли. Свойство варианта
   * лежит в таблице вариантов рядом с `drawerCount`, и новый вариант
