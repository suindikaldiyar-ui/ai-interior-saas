...
const OPEN_PREFIXES = [
  ...
  '/api/complexes/lead',
  /*
   * Демо-страница компании. Адрес уходит мебельной компании письмом, и
   * упереться в общий пароль там — значит получить ответ «у вас ничего
   * не работает» вместо разговора о продукте.
   *
   * ПРЕФИКС СО СЛЕШЕМ. Открывается `/demo/<slug>` — витрина конкретной
   * компании, страница только читает готовое. Голый `/demo` (рабочая
   * демонстрация со сценой, генерацией и правками) остаётся за дверью:
   * `pathname === '/demo'` этому префиксу не равен и его не начинает.
   */
  '/demo/',
];
