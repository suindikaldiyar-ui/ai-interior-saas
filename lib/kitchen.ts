...
export type RunModuleLike = {
  ...
  drawerCount?: number;
  /**
   * Материал фасада: база, конструкция, фактура, цвет.
   *
   * Без него промпт описывает НАПОЛНЕНИЕ точно, а материал — никак, и
   * модель красит гарнитур по своему усмотрению. Филёнчатый фасад без
   * этого блока приезжает гладкой панелью: рисунок рамы и вставки
   * назвать больше нечем.
   */
  front?: {
    base: string;
    construct: string;
