...
/*
 * Список меряется ПО ЗОНЕ, а не целиком: замерщик видит только решения своей
 * зоны, и десять кухонных рядом со шкафными его не касаются.
 *
 * Верхняя граница выросла с восьми до двенадцати вместе с галереей: на
 * встрече клиент говорит «а по-другому можно?», и решений должно быть
 * столько же, сколько в голове у мебельщика. Ниже двух — это не выбор,
 * выше двенадцати — уже каталог, в котором теряются.
 */
for (const zone of ZONE_ORDER) {
  const list = templatesForZone(zone);
  check(
    `${zone}: решений хватает на разговор и не превращается в каталог`,
    list.length >= 2 && list.length <= 12,
    `${list.length} шт.`,
  );
}
